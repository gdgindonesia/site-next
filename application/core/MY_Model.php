...
class MY_Model extends CI_Model
{
    ...
    public function queryGen($table,$item,$action,$where_clause)
  	{
  		...
  		if($action == "C"){
  			...
  			foreach($item as $key => $val)
  			{
  				if(strpos($val,'@lastInsertId') !==false){
  					$value[]=$val;
  				}else{
  					$value[]="'".$val."'";
  				}
  			...
  			foreach($where_clause as $key => $val)
  			{
  				if($no == 1){
  					...
  					$item_clause[] = " And ".$key."='".$val."'";
  				}
  				$no++;
  			}


  			$sql = "UPDATE ".$table." SET ".implode('',$get_item)."";
  			if(count($item_clause)>0){
  				$sql .= " WHERE ";
  				$sql .=implode("",$item_clause);
  			}
  			$sql .=";";
  		}
